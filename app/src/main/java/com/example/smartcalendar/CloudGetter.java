...
public class CloudGetter {
    ...
    public CloudGetter(CloudSender parent) {
        this.database = FirebaseFirestore.getInstance();
        if (parent instanceof IFromCloudGetter) {
            sendData = (IFromCloudGetter) parent;
        } else {
                ...
                if (task.isSuccessful()) {
                    ...
                    for (int i = 0; i < documentList.size(); i++) {
                        ...
                        if ((startMillis > dayStartMillis && startMillis < dayEndMillis)
                            ...
                            boolean smartEvent = (boolean) docData.get("smartEvent");
                            Event event = new Event(title, startTime, endTime, smartEvent, creationMillis);
